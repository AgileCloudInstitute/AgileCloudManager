...
def cleanUp(operation, yaml_infra_config_file_and_path, keyDir, typeName, instanceName, destinationCallInstance, cloud):
  if operation == "on":
    if command_runner.terraformResult == "Applied":
      ...
      source_keys_file_and_path = command_builder.getKeyFileAndPath(keyDir, typeName, cloud)
      ...
      if localBackend:
        ...
        returnedData = command_runner.getShellJsonData(myCommand, destinationCallInstance)
      ...
      if (sp_id == sp_objid) and (sp_id != "empty"):
        app_id = sp_appid
      config_keysassembler.writeTheVarsFile(instanceName, yaml_infra_config_file_and_path, keyDir, cloud, typeName, "admin", secKey, app_id)
  elif operation == "off":
