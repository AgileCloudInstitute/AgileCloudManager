...
#This is the public method that gets called first from the outside
def processAcmConfig():
  quit("BREAKPOINT IN config_validator.py ")
  #First, validate that every system has a unique name in acm.yaml
  typesList = []
  with open('C:\projects\\acm\call\ifc\\acmConfig\\acm.yaml', 'r') as f:
  ...
  for system in acmConfig:
    ...
    if (isinstance(acmConfig.get(system), dict)) and (isinstance(system, str)):
      validateSystem(acmConfig.get(system))
    else:
...
def validateFoundation(foundationDict):
...
def validateImageDefinition(image):
    ...
    for imageKey in image:
      if isinstance(image.get(imageKey), dict):
        ...
        if imageKey == "mappedVariables":
          ...
          validateMappedVariables(image.get(imageKey))
        else:
...
def validateTypeOfService(serviceType):
        ...
        if isinstance(serviceType.get(keyInTypeOfService), dict):
          for varGroupName in serviceType.get(keyInTypeOfService):
            if varGroupName == "mappedVariables":
              validateMappedVariables(serviceType.get(keyInTypeOfService).get(varGroupName))
      ...
      if keyInTypeOfService == "instances":
        ...
        if isinstance(serviceType.get(keyInTypeOfService), list):
          for instance in serviceType.get(keyInTypeOfService):
            validateInstanceOfService(instance)
        else:
...
def validateInstanceOfService(instance):
  ...
  if isinstance(instance, dict):
    ...
    for instanceKey in instance:
      if (instanceKey == "mappedVariables") or (instanceKey == "backendVariables") :
        #mappedVariables section and backendVariables section must be a dictionary of key/value pairs
        validateMappedVariables(instance.get(instanceKey))
      else:
        ...
        if not isinstance(instance.get(instanceKey), str):
          quit("ERROR: All fields within each instance of each type of service must have string values, with the exception of a mappedVariables section, which itself is a dictionary of key/value pairs.  ")
  else:
    quit("ERROR: Each instance of each instance of a serviceType must be a dictionary. ")

def validateMappedVariables(mappedVarsBlock):
